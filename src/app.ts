  ...
  attachCommand('Обгон'),
    ctx.replyWithHTML(
      ...
      Markup.inlineKeyboard([
        ...
        Markup.button.callback('<< Назад', 'Назад'),
      ])
    )
})
...
bot.hears('Скорость', (ctx) => {
  attachCommand('Скорость'), ctx.replyWithHTML(speedScene())
})

bot.hears('Ремень', (ctx) => {
  attachCommand('Ремень'), ctx.replyWithHTML(beltScene())
})

bot.hears('Грязный знак', (ctx) => {
  attachCommand('Грязный знак'), ctx.replyWithHTML(dirtyScene())
})
...
// Actions
bot.action('Назад', (ctx) => ctx.reply('Главное меню', getMainMenu()))
